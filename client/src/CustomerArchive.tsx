...
export default function CustomerArchive({ initialQuery, onOpenInquiries }: { initialQuery?: string; onOpenInquiries: (customerName: string) => void }) {
    ...
    <div className="card">
      ...
      <div className="tablewrap" style={{ overflow: 'auto', maxHeight: '62vh' }}>
        <table className="grid" style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
          <thead><tr>{['客户名称', '国别', '使用地', '来源', '标签', '询价数', '最近询价', '折USD合计', '操作'].map((h) => <th key={h} style={{ background: '#f8fafd', padding: '6px 8px', textAlign: 'left', borderBottom: '1px solid var(--line)', whiteSpace: 'nowrap' }}>{h}</th>)}</tr></thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} style={{ borderBottom: '1px solid var(--line2)' }}>
                <td style={{ padding: '6px 8px', fontWeight: 600 }}>{r.name}</td>
                <td style={{ padding: '6px 8px' }}>{r.country || '—'}</td>
                <td style={{ padding: '6px 8px' }}>{r.use_location || '—'}</td>
                ...
                <td style={{ padding: '6px 8px' }} className="mono">{r.lastDate || '—'}</td>
                <td style={{ padding: '6px 8px' }} className="mono">{money(r.usdTotal)}</td>
                <td style={{ padding: '6px 8px', whiteSpace: 'nowrap' }}>
                  <button className="btn sm" onClick={() => setDetailId(r.id)}>查看档案</button>
                  <button className="btn sm pri" onClick={() => onOpenInquiries(r.name)}>查看询价</button>
                </td>
              </tr>
            ))}
            {rows.length === 0 && <tr><td colSpan={9} style={{ textAlign: 'center', padding: 24, color: 'var(--sub)' }}>暂无客户档案（先到「询报价录入」录一单，即自动建档）</td></tr>}
          </tbody>
        </table>
      </div>
      {detailId && <CustDetailModal id={detailId} onClose={() => setDetailId(null)} onOpenInquiries={onOpenInquiries} />}
    </div>
  )
}

function CustDetailModal({ id, onClose, onOpenInquiries }: { id: string; onClose: () => void; onOpenInquiries: (name: string) => void }) {
      ...
      <div className="modal" style={{ width: 'min(880px, 96vw)', maxHeight: '90vh', overflowY: 'auto' }} role="dialog" aria-modal="true" aria-label="客户档案">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div className="actions" style={{ margin: 0 }}>
            {d && <button className="btn sm pri" onClick={() => { onClose(); onOpenInquiries(d.name) }}>查看该客户询价</button>}
