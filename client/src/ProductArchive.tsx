    ...
    <div className="modal-mask" onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="modal" style={{ width: 'min(1000px, 97vw)', maxHeight: '88vh', overflowY: 'auto' }} role="dialog" aria-modal="true" aria-label="产品价格变动记录">
        ...
        <div className="hint" style={{ marginTop: 6 }}>
          当前参考报价 <b className="mono">{money(p.last_amount)} {p.currency}</b> · 最近数量 <b>{p.last_qty ?? '—'}</b> · 累计使用 {p.use_count} 次 · 当前版本 <b className="mono">V{rows?.length ?? 0}</b>（首次录入 V1，之后每次金额/数量/币种变化生成新版本）
        </div>
