...
export type RangeKey = '' | 'week' | 'lastWeek' | 'month' | 'lastMonth' | 'q3' | 'year'

export const RANGE_LABEL: Record<RangeKey, string> = {
  '': '全部时间', week: '本周', lastWeek: '上周', month: '本月', lastMonth: '上月', q3: '近 3 个月', year: '本年',
}
...
export function rangeDates(key: RangeKey): { from: string; to: string } {
  ...
  if (key === 'lastWeek') {
    ...
    const mon = new Date(now.getFullYear(), now.getMonth(), now.getDate() - dow - 7)
    const sun = new Date(mon.getFullYear(), mon.getMonth(), mon.getDate() + 6)
    return { from: iso(mon), to: iso(sun) }
  }
